(() => {
  ...
  function updateOpeningSky() {
    ...
    const verseDimming = easeBetween(progress, .94, 1);
    const bridgeArrival = easeBetween(progress, .86, .94);
    ...
    root.style.setProperty("--verse-opacity", (verseArrival * (1 - .32 * verseDimming)).toFixed(3));
    root.style.setProperty("--verse-y", `${(2 - 5 * easeBetween(progress, .5, 1)).toFixed(3)}vh`);
    root.style.setProperty("--bridge-opacity", bridgeArrival.toFixed(3));
